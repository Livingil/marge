...
import {
  ...
  formatGoalRewardSummary,
  getCurrentGoal,
  hasGoalRewardSummary
...
test("goal reward is granted only once for the same goal (index 5)", () => {
  ...
  const first = applyGoalRewards(user);
  const second = applyGoalRewards(user);
...
test("reward is not granted for future discovered goal while current unrewarded goal is missing", () => {
  ...
  const reward = applyGoalRewards(user);
...
test("goal reward message is built only when summary has rewards", () => {
  assert.equal(hasGoalRewardSummary({ energy: 0, freeSpawns: 0, freeDeletes: 0 }), false);
  assert.equal(formatGoalRewardSummary({ energy: 50, freeSpawns: 1, freeDeletes: 0 }), "+50 энергии, +1 синтез");

  const user = createUser({
    discoveredItems: [GOAL_SEQUENCE[0]]
  });

  assert.match(applyGoalRewardsAndBuildMessage(user) ?? "", /Цель выполнена/);
});

test("current goal exposes completed state after full discovery", () => {
  const currentGoal = getCurrentGoal([...GOAL_SEQUENCE]);

  assert.equal(currentGoal.reward.energy, 0);
  assert.match(currentGoal.title, /Все цели/);
});
